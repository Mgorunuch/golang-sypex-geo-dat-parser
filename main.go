...
type SxGeo struct {
	...
	mainIndexElementsCount uint16
	oneIndexBlocksCount uint16
	rangesCount uint32
	idBlockSize int
...
func (sxGeo *SxGeo) GetCountryByIp(ip string) string {
	...
	if blockInts[0] - blockInts[1] > uint32(sxGeo.oneIndexBlocksCount) {
		part = sxGeo.searchBlockInMainIndex(
			firstIpByte,
			blockInts[0] / uint32(sxGeo.oneIndexBlocksCount),
			blockInts[1] / uint32(sxGeo.oneIndexBlocksCount) - 1,
		)

		if part > 0 {
			min = part * uint32(sxGeo.oneIndexBlocksCount)
		} else {
			min = 0
		}
		if part > uint32(sxGeo.mainIndexElementsCount) {
			max = sxGeo.rangesCount
		} else {
			max = (part + 1) * uint32(sxGeo.oneIndexBlocksCount)
		}
		if min < blockInts[0] {
			min = blockInts[0]
		}
	...
	if max - min > 1 {
		...
		for true {
			...
			offset := (min + max) >> 1
			if bytes.Compare(nFIP, sxGeo.dbBytes[offset * uint32(sxGeo.blockSize): offset * uint32(sxGeo.blockSize) + 3]) > 0 {
				min = offset
			} else {
				max = offset
			}
		...
		for true {
			if bytes.Compare(nFIP, sxGeo.dbBytes[min * uint32(sxGeo.blockSize): min * uint32(sxGeo.blockSize) + 3]) < 0 {
				break
			}
			min = min + 1
			if min >= max {
	...
	log.Print(len(sxGeo.dbBytes))
	log.Print(len(sxGeo.dbBytes[min * uint32(sxGeo.blockSize - sxGeo.idBlockSize):min * uint32(sxGeo.blockSize)]))

	countryFrom := min * uint32(sxGeo.blockSize) - uint32(sxGeo.idBlockSize)
	countryTo := countryFrom + uint32(sxGeo.idBlockSize)
...
func (sxGeo SxGeo) searchBlockInMainIndex(firstIpByte []byte, min uint32, max uint32) uint32 {
	...
	for true {
		...
		log.Print(
			binary.BigEndian.Uint32(firstIpByte),
			binary.BigEndian.Uint32(newSlice),
		)
...
func NewSxGeo(fileBytes []byte) *SxGeo {
	...
	mainIndexElementsCount := binary.BigEndian.Uint16(fileBytes[11:13])
	mainIndexElementsEnds := uint16(firstBytesEnds) + (mainIndexElementsCount * 4)

	rangesCount := binary.BigEndian.Uint32(fileBytes[15:19])

	idBlockSize := int(fileBytes[19:20][0])
	...
	return &SxGeo{
		...
		oneIndexBlocksCount: binary.BigEndian.Uint16(fileBytes[13:15]),
